...
person_folds <- vfold_cv(person_train, 
                         v = 5,
                         strata = e_i) # 10-fold cross validation
```
...
final_fit <- fit(final_wf, data = person_train)

augment(final_fit, new_data = person_test) %>% 
  roc_auc(truth = e_i, estimate = .pred_Extrovert)

augment(final_fit, new_data = person_test) %>% 
  accuracy(truth = e_i, estimate = .pred_class)
