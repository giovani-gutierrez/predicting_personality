...
pca_pain_wf <- 
  workflow_set(
    preproc = list(pca_pain = pca_pain_rec),
    models = list(elastic_net = elastic_pain_spec,
                  lda = lda_pain_spec,
                  qda = qda_pain_spec,
                  knn = knn_pain_spec)
  )
...
### Second Recipe (`pca_combined_rec`)

#### Elastic Net Logistic Regression
```{r}
elastic_comb_spec <- logistic_reg(mixture = tune(),
                                  penalty = tune()) %>% 
  set_mode('classification') %>% 
  set_engine('glmnet')
```
...
#### K-Nearest Neighbors
```{r}
knn_comb_spec <- nearest_neighbor(neighbors = tune()) %>% 
  set_mode('classification') %>% 
  set_engine('kknn')
```
...
comb_wf <- 
  workflow_set(
    preproc = list(comb = pca_combined_rec),
    models = list(elastic_net = elastic_comb_spec,
                  knn = knn_comb_spec)
  )
```
...
grid_results <-
  write_rds(grid_results, file = 'C:/Users/giova/Desktop/PSTAT 131/predicting_personality/predicting_personality_files/Models/grid_results.rda')
